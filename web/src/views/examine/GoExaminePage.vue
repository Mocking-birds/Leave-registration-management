...
const getGoData = async (state) => {
    ...
    const res = await getGo(pageNum.value, pageSize.value,state)
    console.log(res);
...
const updateData = async (text, data) => {
    ...
    if (text == '驳回') {
        ...
        const res = await updateGo(examineData.value)
        console.log(res);
        getGoData('')
    }
}
            ...
            <el-main>
                <el-scrollbar max-height="750px">
                    <el-table :data="tableData" border style="width: 100%" fit>
                        ...
                        <el-table-column prop="reason" label="出校原因" min-width="180" align="center" />
